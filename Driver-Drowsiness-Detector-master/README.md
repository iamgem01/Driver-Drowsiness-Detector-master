...
### Accuracy tuning (dlib version)
The detector now supports calibration, EAR smoothing, PERCLOS, and 2 alert levels through `config.json`.
...
1. Normal blinking for 20-30 seconds should stay mostly in `NORMAL`.
2. Keep eyes closed for 1-2 seconds to verify `WARNING`.
3. Keep eyes closed longer to verify `CRITICAL` + alarm.
4. Test once in bright light and once in low light.
5. If false alarms happen, lower sensitivity by reducing `threshold_ratio` or increasing `warning_frames`.
6. If alerts are too slow, increase sensitivity by raising `threshold_ratio` or lowering `critical_frames`.
